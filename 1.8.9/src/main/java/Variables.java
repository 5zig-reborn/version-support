...
public class Variables implements IVariables {
	...
	public Object getChatComponentWithPrefix(String prefix, Object originalChatComponent) {
		return new ChatComponentText(prefix).appendSibling((IChatComponent) originalChatComponent);
	}
	...
	public void playSound(String domain, String sound, float pitch) {
		getMinecraft().getSoundHandler().playSound(PositionedSoundRecord.create(new net.minecraft.util.ResourceLocation(domain, sound), pitch));
	}
	...
	public List<NetworkPlayerInfo> getServerPlayers() {
		...
		for (net.minecraft.client.network.NetworkPlayerInfo wrapped : getPlayer().sendQueue.getPlayerInfoMap()) {
	...
	public ResourceLocation getTargetBlockName() {
		...
		return ResourceLocation.fromObfuscated(Block.blockRegistry.getNameForObject(getWorld().getBlockState(blockPosition).getBlock()));
	}
	...
	public String getBiome() {
		...
		Chunk localObject = getMinecraft().theWorld.getChunkFromBlockCoords(localdt);
		return localObject.getBiome(localdt, getMinecraft().theWorld.getWorldChunkManager()).biomeName;
	}
	...
	public boolean isRidingEntity() {
		return getSpectatingEntity().ridingEntity != null;
	}
	...
	public float getResistanceFactor() {
		...
		if (getPlayer().isPotionActive(Potion.resistance)) {
			int resistanceAmplifier = (getPlayer().getActivePotionEffect(Potion.resistance).getAmplifier() + 1) * 5;
		...
		if (referenceDamage <= 0.0F) {
			...
			int enchantmentModifierDamage = EnchantmentHelper.getEnchantmentModifierDamage(getPlayer().getInventory(),
	...
	private PotionEffectImpl wrapPotionEffect(PotionEffect potionEffect) {
		...
		return new PotionEffectImpl(potion == null ? "" : potionEffect.getEffectName(), potionEffect.getDuration(),
				Potion.getDurationString(potionEffect), potionEffect.getAmplifier() + 1, potion == null ? -1 : potion.getStatusIconIndex(),
	...
	public void setSelectedHotbarSlot(int slot) {
		...
		getNetworkManager().sendPacket(new C09PacketHeldItemChange(slot));
	}
	...
	public void messagePlayer(IChatComponent chatComponent) {
		...
		if (!cancel) {
			getPlayer().addChatComponentMessage(chatComponent);
		}
	}

	@Override
	public void sendMessage(String message) {
		getPlayer().sendQueue.addToSendQueue(new C01PacketChatMessage(message));
	}
	...
	public void sendCustomPayload(String channel, ByteBuf payload) {
		if (getNetworkManager() != null) {
			getNetworkManager().sendPacket(new C17PacketCustomPayload(channel, new PacketBuffer(payload)));
		}
	}
	...
	private NetworkManager getNetworkManager() {
		return getMinecraft().getNetHandler() != null ? getMinecraft().getNetHandler().getNetworkManager() : null;
	}
	...
	public MouseOverObject calculateMouseOverDistance(double maxDistance) {
		if (getSpectatingEntity() == null || getWorld() == null)
			return null;
		MovingObjectPosition objectMouseOver = getSpectatingEntity().rayTrace(maxDistance, 1f);
		double var3 = maxDistance;
		Vec3 entityPosition = getSpectatingEntity().getPositionEyes(1f);

		if (objectMouseOver != null) {
			var3 = objectMouseOver.hitVec.distanceTo(entityPosition);
		}

		Vec3 look = getSpectatingEntity().getLook(1f);
		Vec3 mostFarPoint = entityPosition.addVector(look.xCoord * maxDistance, look.yCoord * maxDistance, look.zCoord * maxDistance);
		Entity pointedEntity = null;
		Vec3 hitVector = null;
		List<Entity> entitiesWithinAABBExcludingEntity = getWorld().getEntitiesInAABBexcluding(getSpectatingEntity(),
				getSpectatingEntity().getCollisionBoundingBox().addCoord(look.xCoord * maxDistance, look.yCoord * maxDistance, look.zCoord * maxDistance).expand(1.0, 1.0, 1.0), Predicates.and(EntitySelectors.NOT_SPECTATING, new Predicate<Entity>() {
		...
		for (Entity entity : entitiesWithinAABBExcludingEntity) {
			...
			AxisAlignedBB axisAlignedBB = entity.getCollisionBoundingBox().addCoord((double) collisionBorderSize, (double) collisionBorderSize, (double) collisionBorderSize);
			MovingObjectPosition intercept = axisAlignedBB.calculateIntercept(entityPosition, mostFarPoint);
			if (axisAlignedBB.isVecInside(entityPosition)) {
				if (distance >= 0.0D) {
				...
				if (distanceToHitVec < distance || distance == 0.0D) {
					if (entity == getSpectatingEntity().ridingEntity) {
						if (distance == 0.0D) {
							pointedEntity = entity;
		...
		if (pointedEntity != null && (distance < var3 || objectMouseOver == null)) {
			objectMouseOver = new MovingObjectPosition(pointedEntity, hitVector);
		}
