...
public class FileSelector implements IFileSelector {
	...
	public void draw(int mouseX, int mouseY, float partialTicks) {
		...
		MinecraftFactory.getVars().bindTexture(Gui.optionsBackground);
	...
	protected void bindAmountScrolled() {
		amountScrolled = MathHelper.clamp_float(amountScrolled, 0.0F, (float) this.m());
	}
			...
			if (initialClickY == -1) {
				...
				if (mouseY >= top && mouseY <= bottom) {
					...
					if (mouseX >= scrollBarX0 && mouseX <= scrollBarX1) {
						...
						var8 = MathHelper.clamp_int(var8, 32, bottom - top - 8);
