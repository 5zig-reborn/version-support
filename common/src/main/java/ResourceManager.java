...
public class ResourceManager implements IResourceManager {
				...
				try {
					...
					if (data.models != null) {
						...
						for (TextureData.Model model : models) {
							...
							try {
								Item item = Item.itemRegistry.getObject(new ResourceLocation(model.itemName));
								if (item != null) {
	...
	private IBakedModel bakeModel(ModelBlock modelBlock) {
		SimpleBakedModel.Builder bakedModelBuilder = (new SimpleBakedModel.Builder(modelBlock)).setTexture(TextureAtlasSprite.MISSING_NO);
	...
	private boolean shouldRender(ItemModelResource itemModelResource, EntityPlayer entityPlayer, ItemStack itemStack) {
		if (itemStack.getItem() == itemModelResource.getItem()) {
			if (itemStack.getItem() == Items.bow && entityPlayer.getCurrentEquippedItem() != null) {
				// bow
