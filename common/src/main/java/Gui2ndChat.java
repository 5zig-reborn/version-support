...
public class Gui2ndChat implements IGui2ndChat {
	...
	static {
		try {
			clickChatComponent = GuiScreen.class.getDeclaredMethod(Transformer.REFLECTION.ChatComponentClick().get(), IChatComponent.class);
			...
			hoverChatComponent = GuiScreen.class.getDeclaredMethod(Transformer.REFLECTION.ChatComponentHover().get(),
					IChatComponent.class, int.class, int.class);
	...
	public void draw(int updateCounter) {
		...
		if (MinecraftFactory.getClassProxyCallback().is2ndChatVisible()) {
			...
			if (chatLineCount > 0) {
				...
				int width = MathHelper.ceiling_float_int((float) this.getChatWidth() / chatScale);
				GLUtil.pushMatrix();
				...
				for (lineIndex = 0; lineIndex + this.scrollPos < this.singleChatLines.size() && lineIndex < lineDisplayCount; ++lineIndex) {
					...
					if (chatLine != null) {
						...
						if (ticksPassed < 200 || chatOpened) {
							...
							c = 1.0D - c;
							c *= 10.0D;
							c = MathHelper.clamp_double(c, 0.0D, 1.0D);
							c *= c;
							alpha = (int) (255.0D * c);
							if (chatOpened) {
								alpha = 255;
	...
	private void setChatLine(ChatComponentText chatComponent, int id, int currentUpdateCounter, boolean refresh) {
		...
		int lineWidth = MathHelper.floor_float((float) this.getChatWidth() / this.getChatScale());
		List<IChatComponent> lines = GuiUtilRenderComponents
	...
	private ChatComponentText getChatComponent(int mouseX, int mouseY) {
		if (!this.isChatOpened()) {
			...
			int resolutionScaleFactor = scaledResolution.getScaleFactor();
			float chatScale = this.getChatScale();
			...
			if (x >= 0 && y >= 0) {
				...
				if (x <= MathHelper.floor_float((float) this.getChatWidth() / this.getChatScale() + 3 / getChatScale()) && y < MinecraftFactory.getVars().getFontHeight() * lineCount) {
	...
	private String optStripColor(String text) {
		return GuiUtilRenderComponents.func_178909_a(text, false);
	}
	...
	public int getChatWidth() {
		return MathHelper.floor_float(MinecraftFactory.getClassProxyCallback().get2ndChatWidth());
	}

	public int getChatHeight() {
		return MathHelper.floor_float(this.isChatOpened() ? MinecraftFactory.getClassProxyCallback().get2ndChatHeightFocused() : MinecraftFactory.getClassProxyCallback().get2ndChatHeightUnfocused());
	}
