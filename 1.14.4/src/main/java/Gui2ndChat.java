...
public class Gui2ndChat implements IGui2ndChat {
	...
	static {
		try {
			clickChatComponent = Screen.class.getDeclaredMethod(Transformer.REFLECTION.ChatComponentClick().get(), ITextComponent.class);
	...
	private void setChatLine(StringTextComponent chatComponent, int id, int currentUpdateCounter, boolean refresh) {
		...
		List<ITextComponent> lines = RenderComponentsUtil
	...
	private StringTextComponent getChatComponent(int mouseX, int mouseY) {
		if (!this.isChatOpened()) {
			...
			int resolutionScaleFactor = (int) window.getGuiScaleFactor();
			float chatScale = this.getChatScale();
