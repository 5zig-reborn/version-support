...
public class FaceBakery {
	...
	public BakedQuad a(Vector3f vector1, Vector3f vector2, BlockPartFace blockPartFace, EnumFacing enumFacing, ModelRotation modelRotation,
		...
		return new BakedQuad(data, blockPartFace.tintIndex, facing, null);
	}
	...
	private int a(EnumFacing var) {
		...
		int var2 = MathHelper.clamp((int) (var1 * 255.0F), 0, 255);
	...
	private void a(int[] dataIn, int var1, int count, Vector3f var3, int var4, BlockFaceUV blockPartFace) {
		...
		dataIn[var7 + 3] = var4;
		dataIn[var7 + 4] = Float.floatToRawIntBits(blockPartFace.getVertexU(count) / 16f);
		dataIn[var7 + 4 + 1] = Float.floatToRawIntBits(blockPartFace.getVertexV(count) / 16f);
	}
	...
	private void a(int count, int[] data, EnumFacing enumFacing, BlockFaceUV blockPartFace) {
		...
		if (var6 < -0.1F || var6 >= 1.1F) {
			var6 -= (float) MathHelper.lfloor(var6);
		}

		if (var7 < -0.1F || var7 >= 1.1F) {
			var7 -= (float) MathHelper.lfloor(var7);
		}

		if (var8 < -0.1F || var8 >= 1.1F) {
			var8 -= (float) MathHelper.lfloor(var8);
		}
		...
		int var11 = blockPartFace.getVertexRotatedRev(count) * 7;
