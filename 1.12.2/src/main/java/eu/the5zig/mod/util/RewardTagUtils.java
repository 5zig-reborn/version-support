...
public class RewardTagUtils {
    ...
    public static boolean shouldRender(EntityPlayer player) {
        if(player == Minecraft.getMinecraft().player && !The5zigMod.getConfig().getBool("showOwnNameTag")) return false;
        if(player.isInvisibleToPlayer(Minecraft.getMinecraft().player)) return false;
        if(player.isSneaking()) return false;
        if(player.isBeingRidden()) return false;

        int renderDist = 4096; // 64^2
        if(player.getDistanceSq(Minecraft.getMinecraft().player) > renderDist) return false;
    ...
    public static void render(RenderPlayer renderer, String str, EntityPlayer pl, double x, double y, double z) {
        ...
        GlStateManager.translate((float) x + 0.0F, (float) y + pl.height + 0.35F, (float) z);
