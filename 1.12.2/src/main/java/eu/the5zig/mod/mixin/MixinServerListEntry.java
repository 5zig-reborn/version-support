...
public abstract class MixinServerListEntry {
    ...
    private void redirectCall(int x, int y, float u, float v, int width, int height, float textureWidth, float textureHeight) {
        if(The5zigMod.getConfig().getBool("ping_on_serverlist")) {
            if (server.pingToServer < 0) Minecraft.getMinecraft().fontRenderer.drawString("?", x, y + 1, 0x808079);
            else {
                String pingFormat = String.format("%dms", server.pingToServer);
                x -= (textWidth = Minecraft.getMinecraft().fontRenderer.getStringWidth(pingFormat) / 2);
                Minecraft.getMinecraft().fontRenderer.drawString(pingFormat, x, y + 1, ChatColor.getPingColor(server.pingToServer));
            }
