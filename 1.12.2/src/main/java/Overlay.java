...
public class Overlay implements IOverlay {
	...
	private static class Toast implements IToast {
		...
		@Override
		public IToast.Visibility draw(GuiToast var1, long l) {
			if (updateTime) {
				startedTime = l;
				updateTime = false;
			}
			...
			var1.drawTexturedModalRect(0, 0, 0, overlayTexture * 32, 160, 32);
		...
		public static void create(String title, String subTitle, Object uniqueReference) {
			GuiToast toastFactory = ((Variables) MinecraftFactory.getVars()).getMinecraft().getToastGui();
